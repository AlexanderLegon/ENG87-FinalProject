...
public interface TraineeRepository extends JpaRepository<TraineeEntity, Integer> {
    ...
    @Query(value="SELECT t.first_name, t.last_name, q.quality_gate_status FROM trainee t LEFT JOIN quality_gate q ON t.trainee_id = q.trainee_id WHERE course_id = ?1" , nativeQuery = true)
